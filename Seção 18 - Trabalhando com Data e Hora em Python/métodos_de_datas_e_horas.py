...
hoje_formatado = hoje.strftime('%d/%m/%Y') # %Y -> 0000 | %y -> 00 | %m -> número do mês

hoje_formatado = hoje.strftime('%d/%B/%Y') # %B -> nome do mês em inglês | %b -> iniciais do nome do mês em inglês
...
# pip install blob -> biblioteca muito usada em 2018 para fazer PLN (Processamento de Linguagem Natural)
...
# strptime -> transforma a string em datetime
...
# Testando uma função usando timeit
...
print(timeit.timeit(functools.partial(teste, 2), number=10000)) # Partial -> informa a função e o parâmetro, além do número de vezes
"""
# Testando uma função usando timeit
